'use client';
import Link from "next/link";
import React from 'react';
import Image from "next/image";
import CustomSoftwareMarquee from './CustomSoftwareMarquee';
import { CustomSoftwareHeroSection } from '@/backend/services/services/services.types';

interface CustomSoftwareHeroProps {
  data?: CustomSoftwareHeroSection;
}

export default function CustomSoftwareHero({ data }: CustomSoftwareHeroProps) {
  const subBadgeText = data?.subBadgeText || 'Custom Software Development';
  const headline = data?.headline || 'We build software that fits your business exactly the way it is';
  const subheadline =
    data?.subheadline ||
    'We write custom software that matches exactly how your business runs. Based in Ahmedabad, we help you save time on repetitive work, build something new from the ground up, or fix problems that have been slowing you down.';
  const primaryBtnText = data?.primaryBtnText || 'Get started today';
  const primaryBtnLink = data?.primaryBtnLink || '/about';
  const secondaryBtnText = data?.secondaryBtnText || 'View pricing';
  const secondaryBtnLink = data?.secondaryBtnLink || '/pricing';
  const images = data?.images || {};

  return (
    <section className="rt-hero-v6 rt-position-relative" style={{ overflow: "hidden", paddingLeft: "clamp(1.25rem, 3.5vw, 3rem)", paddingRight: "clamp(1.25rem, 3.5vw, 3rem)" }}>
      <div className="w-layout-blockcontainer rt-container-extra-large rt-position-relative w-container">
        <div className="rt-hero-v6-top rt-desktop-text-center">
          <div
            data-w-id="5f8f0865-2834-ece3-646d-4bbca5ea2ed1"
            className="rt-sub-gap">
            <div className="rt-sub-text rt-sub-gredient">{subBadgeText}</div>
          </div>
          <div className="rt-hero-heading-gap">
            <h1
              data-w-id="5f8f0865-2834-ece3-646d-4bbca5ea2ed4"
              className="rt-gap-off">
              {headline}
            </h1>
          </div>
          <p
            data-w-id="e2966c08-e2ab-594a-a52e-ff609081dee0"
            className="rt-padding-hero-v6 rt-gap-off">
            {subheadline}
          </p>
          <div
            data-w-id="ff501147-2773-073a-9913-6a1e6a9362a7"
            className="w-layout-hflex rt-hero-v7-button-wrap">
            <Link
              data-w-id="7f842da5-19d8-bbc8-1376-5a4231000dc8"
              href={primaryBtnLink}
              className="rt-button-body w-inline-block">
              <div className="rt-button-text">{primaryBtnText}</div>
              <div className="rt-button-body-overlay"></div>
            </Link>
            <Link
              data-wf--rt-border-button--variant="base"
              data-w-id="9067a903-cf07-9614-de57-af0aba677203"
              href={secondaryBtnLink}
              className="rt-button-body rt-nav-btn w-inline-block">
              <div className="rt-button-text rt-btn-color-nav">
                {secondaryBtnText}
              </div>
              <div className="rt-button-body-overlay rt-nav-overlay"></div>
            </Link>
          </div>
        </div>
        <div className="rt-hero-v6-main">
          <div
            data-w-id="bd3b7318-15d0-e2b5-9880-c5981ed75446"
            className="rt-hero-v6-item rt-shadow rt-border-radius-medium">
            <Image
              src={images.hero2 || "/service-2-assets/6909cabf5d40fb1770899a00_taskopia-service-two-hero-2.webp"}
              loading="lazy"
              alt="Custom software hero screen overview"
              width={800} height={800} style={{ width: "100%", height: "auto" }} />
          </div>
          <div className="rt-hero-v6-item rt-two">
            <div
              data-w-id="8a73aa12-aaae-22a9-3807-2177d3e35b77"
              className="rt-hero-v6-iteminner-blue">
              <Image
                src={images.hero1 || "/service-2-assets/6909cabfdfa42ae4bf355d10_taskopia-service-two-hero-1.webp"}
                loading="lazy"
                data-w-id="502bd2f6-e7d7-3b1c-3d3c-6b704526a51e"
                alt="Custom software dashboard preview"
                className="rt-shadow rt-border-radius-medium rt-roted"
                width={800} height={800} style={{ width: "100%", height: "auto" }} />
            </div>
            <div
              data-w-id="b420372a-e374-20ab-e360-50783cf6b4e2"
              className="rt-shadow rt-border-radius-medium">
              <Image
                src={images.heroBannerBottom || "/service-2-assets/69171b49d266be94698645a4_taskopioya-service-two-banner-bottom.png"}
                loading="lazy"
                alt="Custom software banner bottom interface"
                width={800} height={800} style={{ width: "100%", height: "auto" }} />
            </div>
          </div>
          <div className="rt-hero-v6-item rt-two">
            <div
              data-w-id="d180a0b5-6463-e458-42bb-33822b1d9d64"
              className="rt-shadow rt-border-radius-medium">
              <Image
                src={images.hero8 || "/service-2-assets/6909cabfda1a40ef2716a6a3_taskopia-service-two-hero-8.webp"}
                loading="lazy"
                alt="Custom software enterprise reporting"
                width={800} height={800} style={{ width: "100%", height: "auto" }} />
            </div>
            <div
              data-w-id="13027e95-e993-2169-a200-5ecf19604de4"
              className="rt-shadow rt-border-radius-medium">
              <Image
                src={images.hero7 || "/service-2-assets/6909cabfdb82f84467851eea_taskopia-service-two-hero-7.webp"}
                loading="lazy"
                alt="Custom software cloud automation"
                width={800} height={800} style={{ width: "100%", height: "auto" }} />
            </div>
          </div>
          <div className="rt-hero-v6-item rt-two rt-position-relative">
            <div
              data-w-id="90837caa-3eb0-4fd9-23ef-fb0932652d00"
              className="rt-shadow rt-border-radius-medium">
              <Image
                src={images.hero6 || "/service-2-assets/6909cabfa5a17d34fde1fe7d_taskopia-service-two-hero-6.png"}
                loading="lazy"
                alt="Custom software workflows"
                width={800} height={800} style={{ width: "100%", height: "auto" }} />
            </div>
            <div
              data-w-id="328df24a-1ed6-afc3-bcce-1b0c86d62c01"
              className="rt-shadow rt-border-radius-medium">
              <Image
                src={images.hero9 || "/service-2-assets/6909cabf60d8c7cce58ba358_taskopia-service-two-hero-9.webp"}
                loading="lazy"
                alt="Custom software metrics"
                width={800} height={800} style={{ width: "100%", height: "auto" }} />
            </div>
            <div
              data-w-id="d4fa10a8-991b-7efd-da13-e37ad20b1b7f"
              className="rt-hero-v6-item-inner">
              <Image
                src={images.heroRound || "/service-2-assets/6909cabfeee3a35808ad7eb7_Group 2147225566.webp"}
                loading="lazy"
                alt="Custom software badge"
                width={800} height={800} style={{ width: "100%", height: "auto" }} />
            </div>
          </div>
        </div>
      </div>

      <CustomSoftwareMarquee logos={data?.marqueeLogos} />

      <div className="rt-hero-v6-bg">
        <Image
          src={images.bg || "/service-2-assets/69142d3301921d8eace15477_home three hero.webp"}
          loading="lazy"
          alt="home three hero"
          width={800} height={800} style={{ width: "100%", height: "auto" }} />
      </div>
    </section>
  );
}
